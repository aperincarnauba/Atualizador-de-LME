...
def clean_val(v, tipo='metal'):
    ...
    s = str(v).strip().upper()
    if 'FERIADO' in s or s == '-' or s == '': return 'FERIADO'
    try:
        # Metal (Site US): 2,604.00 -> remove vírgula
        if tipo == 'metal': val = s.replace(',', '') 
        # Dolar/Euro (Site BR): 5,40 -> troca vírgula por ponto
...
def processar_planilha(uploaded_file):
    ...
    # 1. Descobrir último ano preenchido (para otimizar scraping)
    ultimo_ano = 2024
    ...
    for row in ws.iter_rows(min_row=3, values_only=True):
        ...
        if dt and dt <= DATA_HOJE:
            if dolar_val is not None and str(dolar_val).strip() != "":
                ultimo_ano = dt.year
            else:
    ...
    for row_idx in range(3, 15000):
        ...
        # Lógica de fim de arquivo (10 vazios seguidos)
        if cell_data is None:
            vazios = 0
            for k in range(1, 10):
                if ws.cell(row=row_idx+k, column=1).value is None: vazios += 1
            if vazios == 9: break
            continue
        ...
        # --- LÓGICA NOVA: FIM DE SEMANA ---
        # 5 = Sábado, 6 = Domingo
        if dt.weekday() in [5, 6]:
            colunas_valores = [2, 4, 6, 8, 10, 12] # B, D, F, H, J, L
            linha_mexida = False
        ...
        if not df_completo.empty and dt in df_completo.index:
            ...
            for col, val in mapeamento.items():
                c = ws.cell(row=row_idx, column=col)
                if val is not None and (c.value is None or str(c.value).strip() == "" or c.value == 0):
                    c.value = val
                    linha_mexida = True
...
if uploaded_file is not None:
    if st.button("🚀 Processar e Atualizar", type="primary"):
        with st.status("Processando...", expanded=True) as status:
            ...
            if processed_data:
                st.write("Baixando dados online...")
                st.write("Identificando finais de semana...")
                st.write("Preenchendo valores...")
                ...
                st.download_button(
                    ...
                    data=processed_data,
                    file_name=f"cotacao_bcb_lme_atualizada_{data_atual}.xlsx",
